...
export class ChaosPlaygroundComponent {
  ...
  readonly statusLabel = computed(() => {
    switch (this.systemState()) {
      case 'healthy':     return 'SYSTEM_OK';
      case 'degraded':    return 'DB_FAULT_DETECTED';
      case 'circuit-open': return 'CIRCUIT_OPEN · FAILOVER_ACTIVE';
    }
  });
  ...
  getEdgeX1(edge: Edge): number { return this.edgeEndpoints(edge).x1; }
  getEdgeY1(edge: Edge): number { return this.edgeEndpoints(edge).y1; }
  getEdgeX2(edge: Edge): number { return this.edgeEndpoints(edge).x2; }
  getEdgeY2(edge: Edge): number { return this.edgeEndpoints(edge).y2; }
