...
@Component({
  ...
  template: `
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-20">
      ...
      <!-- Category filter -->
      <div class="flex flex-wrap gap-3 mb-10" role="group" [attr.aria-label]="i18n.t()('projects.filter-label')">
        <button
...
export default class ProjectsPage implements OnInit {
  ...
  readonly categories = computed(() =>
    [...new Set(this.allProjects().map((p) => p.category).filter((c): c is string => !!c))]
  );
